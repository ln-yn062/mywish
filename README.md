...
<body>
    ...
    <div class="app-container" id="app">
        <!-- Header -->
        <div class="px-6 pt-10 pb-6 shrink-0 bg-white/50 backdrop-blur-sm">
        ...
        <!-- Main Content -->
        <div id="main-content" class="flex-1 overflow-y-auto px-6 pb-24 space-y-10 scrollbar-hide">
            <!-- Dynamic Content -->
        </div>
            ...
            <div class="bg-white w-full p-8 pb-12 animate-slide-up shadow-2xl rounded-t-[32px]">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p id="record-modal-subtitle" class="text-xs text-slate-400 font-medium mt-1">记录下这次努力的心情吧</p>
                    </div>
                ...
                <div class="space-y-6">
                    <div>
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-2 px-1">反馈与感悟</label>
                        <textarea id="input-record-feedback" placeholder="这一刻的感觉如何？" class="w-full bg-slate-50 p-4 rounded-xl text-base font-medium outline-none focus:ring-2 ring-emerald-100 border border-transparent h-32 resize-none"></textarea>
                    </div>
                    <button onclick="confirmRecord()" class="w-full py-4 bg-emerald-600 text-white rounded-xl font-bold text-base shadow-lg active:scale-95 transition-all">确认并保存</button>
                </div>
            </div>
        </div>
        ...
        <div id="modal-add" class="hidden fixed inset-0 z-[60] flex items-end justify-center bg-black/40 backdrop-blur-sm">
            <div class="bg-white w-full p-8 pb-12 animate-slide-up shadow-2xl rounded-t-[32px]">
                ...
                <div class="space-y-6">
                    <div>
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-2 px-1">愿望名称</label>
                    ...
                    <div id="multiple-options" class="hidden">
                        ...
                        <div class="flex items-center gap-4">
                            <input type="number" id="input-item-total" value="20" class="w-20 bg-slate-50 p-3 rounded-xl text-center text-lg font-black text-blue-600 outline-none">
    ...
    <script>
        ...
        let categories = [
            {
                ...
                items: [
                    { 
                        ...
                        completed: false,
                        history: [
                            { time: '2026-01-10 10:00', feedback: '读完了《詹森艺术史》第一章，震撼。' },
                            { time: '2026-02-15 16:20', feedback: '完成第二本，对现代主义有了新理解。' }
                        ]
                    },
                    { id: 102, title: "学会跳萨尔萨舞", type: "once", completed: false, history: [] }
                ]
            },
            {
                id: 2,
                ...
                items: [
                    { 
                        ...
                        type: "once", 
                        completed: true, 
                        completionTime: '2026-03-01 14:30', 
                        feedback: '种了薄荷和绣球，心情非常好！',
                        history: [{ time: '2026-03-01 14:30', feedback: '种了薄荷和绣球，心情非常好！' }]
                    }
                ]
            }
        ];
        ...
        function render() {
            ...
            categories.forEach(cat => {
                ...
                cat.items.forEach(item => {
                    ...
                    mainRow.onclick = (e) => handleItemClick(cat.id, item.id, e);
                    ...
                    mainRow.innerHTML = `
                        ...
                        <div class="flex-1 min-w-0">
                            ${mode === 'edit' 
                                ? `<input onchange="updateItemTitle(${cat.id}, ${item.id}, this.value)" onclick="event.stopPropagation()" class="w-full bg-transparent font-semibold text-slate-800 outline-none" value="${item.title}">`
                            ...
                            ${item.type === 'multiple' ? `
                                <div class="flex items-center gap-2 mt-1">
                                    ...
                                    <span class="text-[8px] text-slate-400 font-bold">${item.current}/${item.total}</span>
                                </div>
                            ` : ''}
                        </div>
                        ...
                        ${mode === 'display' && (item.completed || (item.history && item.history.length > 0)) ? `<i data-lucide="chevron-down" class="w-4 h-4 text-blue-400 transition-transform ${isExpanded ? 'rotate-180' : ''}"></i>` : ''}
                    `;
                        ...
                        let historyHtml = item.history.map((h, idx) => `
                            <div class="flex gap-3 mb-4 last:mb-0">
                                <div class="flex flex-col items-center shrink-0">
                                    ...
                                    ${idx !== item.history.length - 1 ? `<div class="w-px flex-1 bg-slate-200 my-1"></div>` : ''}
                                </div>
                                <div class="flex-1 pb-2">
                                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-tighter mb-1">${h.time}</p>
                                    <p class="text-xs text-slate-600 font-medium leading-relaxed">${h.feedback}</p>
                                </div>
                            </div>
                        `).reverse().join(''); // 最新的显示在上面

                        details.innerHTML = `
                            <div class="px-1 py-2">
                                <h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">打卡历程</h4>
                                ${historyHtml}
                            </div>
                        `;
                ...
                if (mode === 'edit') {
                    ...
                    addBox.innerHTML = `
                        <button onclick="openAddModal(${cat.id}, 'once')" class="flex-1 py-3 bg-white border border-slate-200 rounded-xl flex items-center justify-center gap-2 text-slate-400 text-xs font-bold">
                            <i data-lucide="plus" class="w-4 h-4"></i> 单次
                        </button>
                        <button onclick="openAddModal(${cat.id}, 'multiple')" class="flex-1 py-3 bg-white border border-slate-200 rounded-xl flex items-center justify-center gap-2 text-slate-400 text-xs font-bold">
                            <i data-lucide="plus" class="w-4 h-4"></i> 打卡
                        </button>
                    `;
            ...
            if (mode === 'edit') {
                ...
                addCat.innerHTML = `<i data-lucide="layout-grid" class="w-4 h-4"></i> 添加大类`;
                addCat.onclick = () => {
                    categories.push({ id: Date.now(), name: "新分类", items: [] });
                    render();
                };
        ...
        function handleItemClick(catId, itemId, e) {
            if (mode === 'edit') return;
        ...
        function openRecordModal(catId, itemId) {
            ...
            const subtitle = item.type === 'multiple' 
                ? `第 ${item.current + 1} 次打卡进度` 
                : '终于要完成这个愿望了吗？';
            
            document.getElementById('record-modal-subtitle').textContent = subtitle;
        ...
        function confirmRecord() {
            ...
            const feedbackText = document.getElementById('input-record-feedback').value;
            const cat = categories.find(c => c.id === currentRecordContext.catId);
            const item = cat.items.find(i => i.id === currentRecordContext.itemId);
            
            const now = new Date();
            const timeStr = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-${String(now.getDate()).padStart(2,'0')} ${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`;
            
            const feedback = feedbackText || (item.type === 'once' ? "愿望达成！" : "坚持就是胜利。");
            ...
            expandedItems.add(item.id); // 记录后自动展开
            closeRecordModal();
            render();
        }
        ...
        function updateItemTitle(catId, itemId, val) {
            const cat = categories.find(c => c.id === catId);
            const item = cat.items.find(i => i.id === itemId);
        ...
        function openAddModal(catId, type) {
            ...
            document.getElementById('add-modal-title').textContent = type === 'once' ? '添加单次愿望' : '添加打卡愿望';
        ...
        function confirmAddItem() {
            ...
            if (currentAddContext.type === 'multiple') {
                newItem.total = parseInt(document.getElementById('input-item-total').value) || 20;
            }
